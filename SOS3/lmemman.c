...
bool init_logical_memory(PCB *p, uint32_t code_size) {
	...
	uint32_t n_frames = bytes_to_frames(code_size);

	// 2) allocate frames for above
	// 3) determine beginning physical address of program code,
	//    user-mode stack and kernel-mode stack
	uint32_t program_base = (uint32_t)alloc_frames(n_frames, USER_ALLOC);
	if (program_base == NULL) {
		return FALSE;
	}

	// 4K for kernel stack; 12K for user stack
	// so 1 frame for kernel stack, 3 frames for user stack
	uint32_t kernel_stack = (uint32_t)alloc_frames(1, USER_ALLOC);
	if (kernel_stack == NULL) {
		dealloc_frames((void*)program_base, n_frames);
		return FALSE;
	}
	uint32_t user_stack = (uint32_t)alloc_frames(3, USER_ALLOC);
	if (user_stack == NULL){
		dealloc_frames((void*)program_base, n_frames);
		dealloc_frames((void*)kernel_stack, 1);
		return FALSE;
	}
	...
	// we only have 4GB of storage, so 1 page directory takes 4K
	uint32_t directory_frames = (uint32_t)1;
	// we need 1 page table for each 4M, and 1 more for stack part.
	uint32_t table_frames;
	if ((n_frames >> 10) > 0){
		table_frames = (uint32_t)(1 + (n_frames >> 10));
	}
	else{
		table_frames = (uint32_t)2; // at least 2
	}

	// 5) allocate frames for above
	uint32_t directory_base = (uint32_t)alloc_frames(directory_frames, KERNEL_ALLOC);
	if (directory_base == NULL){
		dealloc_frames((void*)program_base, n_frames);
		dealloc_frames((void*)kernel_stack, (uint32_t)1);
		dealloc_frames((void*)user_stack, (uint32_t)3);
		return FALSE;
	}
	uint32_t tables_base = (uint32_t)alloc_frames(table_frames, KERNEL_ALLOC);
	if (tables_base == NULL){
		dealloc_frames((void*)program_base, n_frames);
		dealloc_frames((void*)kernel_stack, (uint32_t)1);
		dealloc_frames((void*)user_stack, (uint32_t)3);
		dealloc_frames((void*)directory_base, directory_frames);
		return FALSE;
	}
	// 6) set up page directory and page tables
	//set up page directory
	PDE *directory = (PDE *)(directory_base + KERNEL_BASE);
	PTE *tables = (PTE *)(tables_base + KERNEL_BASE);
	//initialize to 0
	uint32_t i;
	for (i = 0; i < 1024; i++) directory[i] = 0;
	for (i = 0; i<1024 * table_frames; i++) tables[i] = 0;

	//set page directory/table entry for code and data
	for (i = 0; i < (uint32_t)(table_frames - 1); i++){
		directory[i] = (uint32_t) (tables_base + (i * 4096)) | PDE_PRESENT | PDE_READ_WRITE | PDE_USER_SUPERVISOR;
	}

	for (i = 0; i < n_frames; i++){
		tables[i] = (uint32_t)((program_base + (i * 4096)) | PTE_PRESENT | PTE_READ_WRITE | PTE_USER_SUPERVISOR);
	}

	//set page directory/table entry for stacks
	directory[766] = (uint32_t)(tables_base + (table_frames - 1) * 4096) | PDE_PRESENT | PDE_READ_WRITE | PTE_USER_SUPERVISOR;

	PTE *pages_766 = (PTE *)(tables_base + (table_frames - 1)*4096 + KERNEL_BASE);
	pages_766[1020] = (PTE)((user_stack) | PTE_PRESENT | PTE_READ_WRITE | PTE_USER_SUPERVISOR);
	pages_766[1021] = (PTE)((user_stack + 4096) | PTE_PRESENT | PTE_READ_WRITE | PTE_USER_SUPERVISOR);
	pages_766[1022] = (PTE)((user_stack + 8192) | PTE_PRESENT | PTE_READ_WRITE | PTE_USER_SUPERVISOR);
	pages_766[1023] = (PTE)((kernel_stack) | PTE_PRESENT | PTE_READ_WRITE | PTE_USER_SUPERVISOR);

	directory[768] = k_page_directory[768];
	...
	p->mem.start_brk = n_frames * 4096;
	p->mem.brk = n_frames * 4096;;
	p->mem.page_directory = (PDE*)directory_base;
